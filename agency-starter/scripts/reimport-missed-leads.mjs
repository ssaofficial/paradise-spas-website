 ...
 *
 * Usage:
 *   GHL_API_TOKEN=pit-... GHL_LOCATION_ID=your-location-id \
 *     node scripts/reimport-missed-leads.mjs "Missed Leads.csv"
 *
...
function buildPayload(row, locationId, forCreate) {
  var names = splitName(row.full_name);
  var payload = {
    firstName: names.firstName,
    lastName: names.lastName || '.',
    email: String(row.email || '').trim().toLowerCase(),
    phone: '+1' + String(row.phone || '').replace(/\D/g, '').slice(-10),
    source: 'Website — missed-lead-reimport',
    tags: ['website-form', 'lead-api', 'missed-lead-reimport']
  };
...
async function main() {
  ...
  if (!token || !locationId) {
    console.error('Set GHL_API_TOKEN and GHL_LOCATION_ID');
    process.exit(1);
  }
