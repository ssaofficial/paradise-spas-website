...
function leadValue(env) {
  var v = parseFloat(env.LEAD_VALUE_USD || '500');
...
export async function sendLeadEvent(env, request, lead, meta) {
  ...
  var payload = {
    data: [{
      ...
      event_source_url: lead.pageUrl || env.ALLOWED_ORIGIN || '',
      user_data: userData,
      custom_data: {
        value: value,
